...
// Set carry flag
static inline int8_t set_CF(int new_CF)
{
	return regs8[FLAG_CF] = !!new_CF;
}

// Set auxiliary flag
static inline int8_t set_AF(int new_AF)
{
	return regs8[FLAG_AF] = !!new_AF;
}

// Set overflow flag
static inline int8_t set_OF(int new_OF)
{
	return regs8[FLAG_OF] = !!new_OF;
}

// Set auxiliary and overflow flag after arithmetic operations
static inline int8_t set_AF_OF_arith()
{
...
static inline void set_opcode(uint8_t opcode)
{
...
inline static __attribute__((always_inline)) int8_t pc_interrupt(uint8_t interrupt_num)
{
...
static inline int AAA_AAS(int8_t which_operation)
{
	return (regs16[REG_AX] += 262 * which_operation*set_AF(set_CF(((regs8[REG_AL] & 0x0F) > 9) || regs8[FLAG_AF])), regs8[REG_AL] &= 0x0F);
}

static inline void mmcpy(void* to, void* from, unsigned int len)
{
    uint8_t* a = (uint8_t*)to;
    uint8_t* b = (uint8_t*)from;
    for (unsigned int i = 0; i < len; i++) a[i] = b[i];
}
...
int main(void)
{
	...
	// But, if the HD image file is prefixed with @, then boot from the HD
	regs8[REG_DL] = 0;
	CAST(unsigned)regs16[REG_AX] = 0;

	// Load BIOS image into F000:0100, and set IP to 0100
	reg_ip = 0x100;
	mmcpy(regs8 + reg_ip, bios, bios_len);
#endif
	...
	unsigned int cyc = 0, seconds = 1234789;
...
//						uint8_t* tmp = mem + SEGREG(REG_ES, REG_BX,);
						if (++millis >= 1000) {
							...
							millis += 1;
						//mmcpy(tmp, localtime(&seconds), sizeof(seconds));
					...
					OPCODE_CHAIN 3: // DISK_WRITE
						regs8[REG_AL] = ~lseek(disk[regs8[REG_DL]], CAST(unsigned)regs16[REG_BP] << 9, 0)
							? ((int8_t)i_data0 == 3 ? (int(*)())write : (int(*)())read)(disk[regs8[REG_DL]], mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX])
							: 0;
#else
                    OPCODE 2: // DISK_READ
                        if ((regs8[REG_DL] == 1) && (CAST(unsigned)regs16[REG_BP] << 9) < fd_img_len) {
                            unsigned int off = CAST(unsigned)regs16[REG_BP] << 9;
                            mmcpy(mem + SEGREG(REG_ES, REG_BX,), fd_img+off, regs16[REG_AX]);
                            regs8[REG_AL] = regs16[REG_AX];
                        } else
                            regs8[REG_AL] = 0;
                    ...
                    OPCODE 3: // DISK_WRITE
                        if ((regs8[REG_DL] == 1) && (CAST(unsigned)regs16[REG_BP] << 9) < fd_img_len) {
                            unsigned int off = CAST(unsigned)regs16[REG_BP] << 9;
                            mmcpy(fd_img+off, mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX]);
                            regs8[REG_AL] = regs16[REG_AX];
                        } else
                            regs8[REG_AL] = 0;
#endif
